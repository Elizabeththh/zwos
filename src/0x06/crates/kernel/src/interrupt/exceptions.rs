...
pub extern "x86-interrupt" fn bound_range_exceeded_handler(stack_frame: InterruptStackFrame) {
    panic!(
        "EXCEPTION: BOUND_RANGE_EXCEEDED_ERROR\n\n{:#?}",
        stack_frame
    );
}
...
pub extern "x86-interrupt" fn invalid_tss_handler(
    ...
    panic!(
        "EXCEPTION: INVALID TSS, ERROR CODE: 0x{:016x}\n\n{:#?}",
...
pub extern "x86-interrupt" fn segment_not_present_handler(
    ...
    panic!(
        "EXCEPTION: SEGMENT NOT PRESENT, ERROR CODE: 0x{:016x}\n\n{:#?}",
        error_code, stack_frame
    );
}
...
pub extern "x86-interrupt" fn stack_segment_fault_handler(
    ...
    panic!(
        "EXCEPTION: STACK SEGMENT FAULT, ERROR CODE: 0x{:016x}\n\n{:#?}",
        error_code, stack_frame
    );
}
...
pub extern "x86-interrupt" fn general_protection_fault_handler(
    ...
    match error_code {
        0 => panic!(
            "EXCEPTION: GENERAL PROTECTION FAULT(SEGMENT RELATED), ERROR CODE: 0x{:016x}\n\n{:#?}",
            error_code, stack_frame
        ),
        _ => panic!("EXCEPTION: GENERAL PROTECTION FAULT\n\n{:#?}", stack_frame),
    }
}
...
pub extern "x86-interrupt" fn page_fault_handler(
    ...
    if !crate::proc::handle_page_fault(
        ...
        let name =
            crate::proc::current_process_name_safe().unwrap_or_else(|| "unknown".to_string());
...
pub extern "x86-interrupt" fn x87_floating_point_handler(stack_frame: InterruptStackFrame) {
    panic!(
        "EXCEPTION: x86 FLOATING POINT EXCEPTION\n\n{:#?}",
        stack_frame
    );
}
...
pub extern "x86-interrupt" fn alignment_check_handler(
    ...
    panic!(
        "EXCEPTION: ALIGNMENT CHECK, ERROR CODE: 0x{:016x}\n\n{:#?}",
        error_code, stack_frame
    );
}
